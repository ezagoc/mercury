...
for (stage in list('stage3_4')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    pvals = read_excel(paste0("../../../data/04-analysis/", country, 
                              "/",stage,"/",type,
                              "agg_pvalues_all_country_b1b2p_sent.xlsx"))
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_sent_b1b2p_all_",type,stage),
      header = FALSE,
      ...
      title = paste0('Panel C, Pooled Sentiment (VADER) Analysis Results (Pilot, Batch 1 and 2) for the SMI Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex") %>% 

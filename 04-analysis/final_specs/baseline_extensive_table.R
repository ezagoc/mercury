...
for (stage in list('baseline')){
  for (type in list('', 'log_', 'arc_')){
    ...
    df_ke <- read_parquet(paste0('../../../data/04-analysis/KE/stage1_2/final_data_b1b2p.parquet')) |> mutate(pais = 'KE')
    
    df_ke <- df_ke |> mutate(total_treated = t_strong + t_weak + t_neither,
                             total_influencers = c_t_strong_total + c_t_weak_total + 
                               c_t_neither_total) |> 
      filter(total_influencers == 1) |> filter(n_posts_base > 0)
    ...
    pvals = read_excel(paste0("../../../data/04-analysis/", country, 
                              "/",stage,"/", type,
                              "agg_pvalues_b1b2p_extensive.xlsx"))
    ...
    table <- stargazer(
      ...
      label = paste0("tab:baseline_extensive_b1b2p",stage,"", country),
      header = FALSE,
      ...
      add.lines = list(c('Block1 FEs', rep('Yes', 8)), c("Outcome mean", means),

  ...
  for (type in list('', 'log_', 'arc_')){ 
    ...
    df <- df |> mutate(total_treated = t_strong + t_weak + t_neither,
                       total_influencers = c_t_strong_total + c_t_weak_total + 
                         c_t_neither_total,
                       strat_block1 = paste0(strat_block1, pais))
    ...
    # Load pvalues obtained through 1000 permutations of assigment
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_sent_bert_b1_ads_",type,stage),
      header = FALSE,
      ...
      title = paste0('Panel B, Pooled Sentiment (BERT) Analysis Results (Batch 1) for the ADs Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex")
    
    note.latex <- paste0("\\multicolumn{17}{l} {\\parbox[t]{24cm}{ \\textit{Notes:}
The unit of observation is an influencer's follower. Users with no Twitter activity are coded as zeros.
We report estimates from OLS regression. Robust standard errors are in parentheses. 
 * denotes p$<$0.1, ** denotes p$<$0.05, and *** denotes p$<$0.01.}} \\\\")
    table[grepl("Note", table)] <- note.latex
    print(table)
    cat(table, file = paste0("../../../results/04-analysis/new_ads/", type,
                             "endline_table_sent_bert_b1_all_",
                             stage,".tex"))

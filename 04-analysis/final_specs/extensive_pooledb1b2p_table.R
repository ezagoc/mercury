...
for (stage in list('stage1_2')){
  for (type in list('', 'log_', 'arc_')){
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_b1b2p_ext",stage,"", country),
      header = FALSE,
      ...
      add.lines = list(c("Baseline control", rep("Yes", 8)),
                       c("Block1 FEs", rep("Yes", 8)),
                       c("Outcome mean", means),
                       c("Outcome std. dev.", sds),
                       c("Outcome range", range)),
      title = paste0('Panel D, Pooled (Batch 1, 2 and Pilot) Extensive Endline Results for the SMI Treatment for Both Countries, (',
                     weeks,')'),
      type = "latex")  %>% 

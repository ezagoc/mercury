...
for (stage in list('stage3_4', 'stage5_6')){
  for (type in list('', 'log_', 'arc_')){
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_b1b2p_extensive_",stage),
      header = FALSE,
      ...
      omit = c('block1_fe', paste0(aux, '_base')),
      omit.stat=c("f", "ser","adj.rsq"),
      column.sep.width = "0pt",
      add.lines = list(c('Block 1 FEs', rep('Yes', 8)),
                       c("Baseline control", rep("Yes", 8)),
                       c("Outcome mean", means),
                       c("Outcome std. dev.", sds),
                       c("Outcome range", range)),
      title = paste0('Panel D, Pooled (Batch 1, 2 and Pilot) Endline Extensive Results for the SMI Treatment for Both Countries, (',
                     weeks,')'),
      type = "latex")  %>% 
      star_insert_row(insert.after=13, paste0(" & ", pvals, " \\\ "))
    
    note.latex <- paste0("\\multicolumn{9}{l} {\\parbox[t]{16cm}{ \\textit{Notes:}
The unit of observation is an influencer's follower. This is an extensive analysis, meaning we are only comparing followers that only follow one influencer. 
We report estimates from OLS regression.
Specifications further include block1 randomization of the influencers fixed effects. Robust standard errors are in parentheses. 

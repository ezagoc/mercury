...
for (stage in list('stage5_6')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_sent_bert_b1b2p_ext", type, stage),
      header = FALSE,
      font.size = "tiny",
      dep.var.caption = "",
      ...
      add.lines = list(c("Block 1 FEs", rep('Yes', 16)),
                       c("Baseline control", rep("Yes", 16)),
                       c("Outcome mean", means),
                       c("Outcome std. dev.", sds),
                       c("Outcome range", range)),
      title = paste0('Panel C, Pooled Sentiment (BERT) Extensive Analysis Results (Pilot, Batch 1 and 2) for the SMI Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex") %>% 

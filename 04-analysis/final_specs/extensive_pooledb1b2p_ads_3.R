...
for (stage in list('stage5_6')){
  for (type in list('', 'log_', 'arc_')){
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_b1b2p_ads",stage),
      header = FALSE,
      ...
      add.lines = list(c("Baseline control", rep("Yes", 8)),
                       c("Strat. Block 1 FEs", rep("Yes", 8)),
                       c("Outcome mean", means),
                       c("Outcome std. dev.", sds),
                       c("Outcome range", range)),
      title = paste0('Panel B, Pooled (Batch 1, 2 and Pilot) Endline Extensive Results for the ADs Treatment for Both Countries, (',
                     weeks,')'),
      type = "latex")
    
    note.latex <- paste0("\\multicolumn{9}{l} {\\parbox[t]{16cm}{ \\textit{Notes:}
The unit of observation is an influencer's follower. Users without any Twitter activity are coded as zeros. This is an extensive analysis, 
meaning we are only comparing followers that only follow one influencer.

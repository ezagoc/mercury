...
for (stage in list('stage1_2')){
  for (type in list('', 'log_', 'arc_')){
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_extensive_b1b2p_ads",stage),
      header = FALSE,
      ...
      title = paste0('Panel D, Pooled (Batch 1, 2 and Pilot) Endline Results for the ADs Treatment for Both Countries, (',
                     weeks,')'),
      type = "latex")
...
This is an extensive analysis, meaning we are only comparing followers that only follow one influencer. We report estimates from OLS regression. 

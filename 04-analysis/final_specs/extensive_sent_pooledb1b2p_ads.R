...
for (stage in list('stage1_2', 'stage3_4', 'stage5_6')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_sent_b1b2p_ads_", type, stage),
      header = FALSE,
      font.size = "tiny",
      dep.var.caption = "",
      ...
      title = paste0('Panel B, Pooled Sentiment (VADER) Analysis Extensive Results (Pilot, Batch 1 and 2) for the ADs Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex")

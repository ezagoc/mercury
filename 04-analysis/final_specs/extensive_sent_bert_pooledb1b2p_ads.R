...
for (stage in list('stage1_2')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_sent_bert_b1b2p_ads_", type, stage),
      header = FALSE,
      ...
      title = paste0('Panel B, Pooled Sentiment (BERT) Extensive Analysis Results (Pilot, Batch 1 and 2) for the ADs Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex")
...
df_prueba <- df |> filter(batch_id != 'p')

...
for (stage in list('stage1_2', 'stage3_4', 'stage5_6')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    pvals = read_excel(paste0("../../../data/04-analysis/", country, 
                              "/",stage,"/",type,
                              "agg_pvalues_all_country_b1_sent_bert.xlsx"))
    ...
    table <- stargazer(
      ...
      label = paste0("tab:endline_table_sent_bert_b1_all_",type,stage),
      header = FALSE,
      ...
      title = paste0('Panel C, Pooled Sentiment (BERT) Analysis Results (Batch 1) for the SMI Treatment in Both Countries', 
                     '(',weeks,')'),
      type = "latex") %>% 

...
df_stages <- paste0(rep('stage', 6), c(1:6)) |> 
  map_dfr(~get_panel_stages(.x, path_normal, 'sent_bert', country))

df_stages <- df_stages |> select(-c("__index_level_0__"))

write_parquet(df_stages, paste0(path_normal, country, '/sentiment_panel_bert_b1.parquet'))

base <- df_stages |> select(username:n_posts_no_rt_vax_base, stage_id) |> 
  filter(stage_id == 'stage1') |> select(-stage_id)

### Divide by Stages:
## 1-2

stage1_2 <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
  filter(stage_id == 'stage1' | stage_id == 'stage2') |> group_by(follower_id) |>
  summarise(across(c(pos_b_rt:n_posts_no_rt_vax), ~sum(.x))) |> ungroup()

stage1_2 <- base |> left_join(stage1_2, by = 'follower_id')

write_parquet(stage1_2, paste0(path_normal, country, 
                               '/stage1_2/endline_sent_bert_b1.parquet'))

stage1_2 <- stage1_2 |> mutate(batch_id = 'b1')
## 3-4

stage3_4 <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
...
write_parquet(stage3_4, paste0(path_normal, country, 
                               '/stage3_4/endline_sent_bert_b1.parquet'))

stage3_4 <- stage3_4 |> mutate(batch_id = 'b1')
## 5-6

stage5_6 <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
...
write_parquet(stage5_6, paste0(path_normal, country, 
                               '/stage5_6/endline_sent_bert_b1.parquet'))
...
df_stages <- paste0(rep('stage', 2), c(1:2)) |> 
  map_dfr(~get_panel_stages(.x, path_normal, 'sent_bert_batch2', country))

stage1_2_b2 <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
  filter(stage_id == 'stage1' | stage_id == 'stage2') |> group_by(follower_id) |>
  summarise(across(c(pos_b_rt:n_posts_no_rt_vax), ~sum(.x))) |> ungroup()
...
write_parquet(stage1_2_3, paste0(path_normal, country, 
                               '/stage1_2/endline_sent_bert_b1_b2.parquet'))


df_stages <- paste0(rep('stage', 2), c(1:4)) |> 
  map_dfr(~get_panel_stages(.x, path_normal, 'sent_bert_pilot', country))

stage1_2_p <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
  filter(stage_id == 'stage1' | stage_id == 'stage2') |> group_by(follower_id) |>
  summarise(across(c(pos_b_rt:n_posts_no_rt_vax), ~sum(.x))) |> ungroup()
...
write_parquet(stage1_2_3, paste0(path_normal, country, 
                                 '/stage1_2/endline_sent_bert_b1_b2_p.parquet'))

## 3 4 WITH BATCH 2

stage3_4_p <- df_stages |> select(follower_id, pos_b_rt:stage_id) |> 
  filter(stage_id == 'stage3' | stage_id == 'stage4') |> group_by(follower_id) |>
  summarise(across(c(pos_b_rt:n_posts_no_rt_vax), ~sum(.x,na.rm = T))) |> ungroup() 

base <- df_stages |> select(username:n_posts_no_rt_vax_base, stage_id) |> 
  filter(stage_id == 'stage1') |> select(-stage_id)

stage3_4_p <- base |> left_join(stage3_4_p, by = 'follower_id') |> mutate(batch_id = 'p')

# Import batch 2:

stage3_4_batch2 <- read_parquet(paste0(path_normal, country, 
                                       '/stage3_4/endline_sent_bert_batch2.parquet')) |> 
  select(-c("__index_level_0__")) |> mutate(batch_id = 'b2')

stage1_2_3 <- rbind(stage3_4_batch2, stage3_4_p, stage3_4)

write_parquet(stage1_2_3, paste0(path_normal, country, 
                                 '/stage3_4/endline_sent_bert_b1_b2_p.parquet'))

# 5 - 6 First just without pilot

stage5_6_batch2 <- read_parquet(paste0(path_normal, country, 
                                       '/stage5_6/endline_sent_bert_batch2.parquet')) |> 
  select(-c("__index_level_0__")) |> mutate(batch_id = 'b2')
...
write_parquet(stage5_6_1, paste0(path_normal, country, 
                                 '/stage5_6/endline_sent_bert_b1_b2_p.parquet'))

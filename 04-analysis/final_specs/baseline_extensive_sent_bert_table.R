...
for (stage in list('baseline')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    pvals = read_excel(paste0("../../../data/04-analysis/", country, 
                              "/",stage,"/",type,
                              "agg_pvalues_b1b2p_extensive_sent_bert.xlsx"))
    ...
    table <- stargazer(
      ...
      label = paste0("tab:baseline_sent_bert_b1b2p_extensive",type,stage),
      header = FALSE,
...
Specifications further include block 1 randomization of the influencer fixed effects. 
Robust standard errors are in parentheses. 
 * denotes p$<$0.1, ** denotes p$<$0.05, and *** denotes p$<$0.01.}} \\\\")
    table[grepl("Note", table)] <- note.latex
    print(table)
    cat(table, file = paste0("../../../results/04-analysis/extensive/", type,
                             "baseline_table_sent_bert.tex"))

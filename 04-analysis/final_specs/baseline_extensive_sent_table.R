...
for (stage in list('baseline')){
  for (type in list('', 'log_', 'arc_')){ 
    ...
    pvals = read_excel(paste0("../../../data/04-analysis/", country, 
                              "/",stage,"/",type,
                              "agg_pvalues_b1b2p_extensive_sent.xlsx"))
    ...
    table <- stargazer(
      ...
      label = paste0("tab:baseline_sent_b1b2p_extensive_",type, stage),
      header = FALSE,
      font.size = "tiny",
      dep.var.caption = "",
      ...
      omit = c('block1_fe', 'Constant'),
      omit.stat=c("f", "ser","adj.rsq"),
      column.sep.width = "0pt",
      add.lines = list(c('Block1 FEs', rep('Yes', 16)),
                       c("Outcome mean", means),
                       c("Outcome std. dev.", sds),
                       c("Outcome range", range)),
      title = paste0('Panel C, Pooled Sentiment (VADER) Analysis Baseline Results (Pilot, Batch 1 and 2) for the SMI Treatment in Both Countries'),
      type = "latex") %>% 
      star_insert_row(insert.after=13, paste0(" & ", pvals, " \\\ "))
    
    note.latex <- paste0("\\multicolumn{17}{l} {\\parbox[t]{24cm}{ \\textit{Notes:}
The unit of observation is an influencer's follower. The analysis is restricted to followers that posted at least one 
Tweet at baseline and follow only one influencer. We report estimates from OLS regression.
Specifications further include block 1 randomization of the influencer fixed effects. Robust standard errors are in parentheses. 
 * denotes p$<$0.1, ** denotes p$<$0.05, and *** denotes p$<$0.01.}} \\\\")
    table[grepl("Note", table)] <- note.latex
    print(table)
    cat(table, file = paste0("../../../results/04-analysis/extensive/", type,
                             "baseline_extensive_sent_b1b2p.tex"))

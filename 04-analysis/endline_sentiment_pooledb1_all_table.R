...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_sent_b1_all_",stage),
  header = FALSE,
  ...
  title = paste0('Panel C, Pooled Sentiment Analysis Results (Batch 1) for the SMI Treatment in ',country, 
                 ', (',weeks,')'),
  type = "latex") %>% 
...
cat(table, file = paste0("../../results/04-analysis/", stage,
                         "/endline_table_sent_b1_all_", country,".tex"))
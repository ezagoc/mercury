...
df_KE <- df_KE |> mutate(strat_block1 = paste0(strat_block1, 'KE'))

df_SA <- read_parquet(paste0('../../data/04-analysis/SA/', stage,
                             '/endline_sent_b1.parquet'))

df_SA <- df_SA |> mutate(strat_block1 = paste0(strat_block1, 'SA'))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_ads_sent_b1_",stage,"", country),
  header = FALSE,
  ...
  title = paste0('Panel B, Sentiment Analysis Results for the Ads Treatment for Both Countries (', 
                 weeks,')'),
  type = "latex")
...
print(table)
cat(table, file = paste0("../../results/04-analysis/",country,"/endline_ads_sent_b1_", 
                         stage,".tex"))
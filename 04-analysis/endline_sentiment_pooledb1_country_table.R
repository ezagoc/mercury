...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_sent_b1_country_",stage),
  header = FALSE,
  ...
  title = paste0('Panel A, Pooled Sentiment Analysis Results (Batch 1) for the SMI Treatment in Both Countries, (',
                 weeks,')'),
  type = "latex")  %>% 
...
cat(table, file = paste0("../../results/04-analysis/", country,
                         "/endline_table_sent_b1_", stage,".tex"))
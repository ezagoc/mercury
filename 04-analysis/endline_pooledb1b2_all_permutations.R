...
for (m in 1:4){
  ...
  ver_rt <- ver_rt[1,] %>% flatten_chr()
  
  ver_rt_f <- coefs_all %>% 
    select(ends_with(aux[2]))
  ver_rt_f <- ver_rt_f[1,] %>% flatten_chr()
  
  true_rt <- coefs_all %>% 
    select(ends_with(aux[3]))
  true_rt <- true_rt[1,] %>% flatten_chr()
  
  true_rt_f <- coefs_all %>% 
    select(ends_with(aux[4]))
  true_rt_f <- true_rt_f[1,] %>% flatten_chr()
  
  fake_rt <- coefs_all %>% 
    select(ends_with(aux[5]))
  fake_rt <- fake_rt[1,] %>% flatten_chr()
  
  fake_rt_f <- coefs_all %>% 
    select(ends_with(aux[6]))
  fake_rt_f <- fake_rt_f[1,] %>% flatten_chr()
  
  n_posts_rt <- coefs_all %>% 
    select(ends_with(aux[7]))
  n_posts_rt <- n_posts_rt[1,] %>% flatten_chr()
  
  n_posts_rt_f <- coefs_all %>% 
    select(ends_with(aux[8]))
  n_posts_rt_f <- n_posts_rt_f[1,] %>% flatten_chr()
  
  ver_no_rt <- coefs_all %>% 
    select(ends_with(aux[9]))
  ver_no_rt <- ver_no_rt[1,] %>% flatten_chr()
  
  ver_no_rt_f <- coefs_all %>% 
    select(ends_with(aux[10]))
  ver_no_rt_f <- ver_no_rt_f[1,] %>% flatten_chr()
  
  true_no_rt <- coefs_all %>% 
    select(ends_with(aux[11]))
  true_no_rt <- true_no_rt[1,] %>% flatten_chr()
  
  true_no_rt_f <- coefs_all %>% 
    select(ends_with(aux[12]))
  true_no_rt_f <- true_no_rt_f[1,] %>% flatten_chr()
  
  fake_no_rt <- coefs_all %>% 
    select(ends_with(aux[13]))
  fake_no_rt <- fake_no_rt[1,] %>% flatten_chr()
  
  fake_no_rt_f <- coefs_all %>% 
    select(ends_with(aux[14]))
  fake_no_rt_f <- fake_no_rt_f[1,] %>% flatten_chr()
  
  n_posts_no_rt <- coefs_all %>% 
    select(ends_with(aux[15]))
  n_posts_no_rt <- n_posts_no_rt[1,] %>% flatten_chr()
  
  n_posts_no_rt_f <- coefs_all %>% 
    select(ends_with(aux[16]))
  n_posts_no_rt_f <- n_posts_no_rt_f[1,] %>% flatten_chr()
  ...
  write_xlsx(coefs_perm, paste0("../../data/04-analysis/",country, "/",
                                stage, "/pestimates_b1_b2_fake_all", i,".xlsx"))
  i <- i + 1
}

...
aux <- c('verifiability_rt_base', 'true_rt_base', 'n_posts_rt_base',
         'verifiability_no_rt_base', 'true_no_rt_base', 'n_posts_no_rt_base')
...
for (x in aux) {
  fmla1 <- as.formula(paste0(x, "~ t_strong + t_weak + t_neither | 
                             c_t_strong_total + c_t_weak_total + 
                             c_t_neither_total"))
...
table <- stargazer(
  ...
  label = paste0("tab:baseline_table_", country),
  header = FALSE,
  ...
  title = paste0('Panel A, Baseline for the SMI Treatment in ', country),
  type = "latex")  %>% 
...
print(table)
cat(table, file = paste0("../../results/04-analysis/baseline_table_", 
                         country,".tex"))
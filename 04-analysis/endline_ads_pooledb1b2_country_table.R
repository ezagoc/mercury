...
data <- data |> mutate(strat_block1 = paste0(strat_block1, pais))
...
table <- stargazer(
  ...
  label = paste0("tab:pooled_ads_b1_b2_fake_",stage,"", country),
  header = FALSE,
  ...
  title = paste0('Panel B, Pooled (Batch 1) Endline Results for the Ads Treatment for Both Countries ', 
                 '(', weeks,')'),
  type = "latex")
...
print(table)
cat(table, file = paste0("../../results/04-analysis/", country, "/endline_ads_b1_b2_fake_", 
                         stage,".tex"))
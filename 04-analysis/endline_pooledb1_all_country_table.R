...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_b1_fake_all.xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_b1_fake_all",stage,"", country),
  header = FALSE,
  ...
  title = paste0('Panel C, Pooled (Batch 1) Endline Results for the SMI Treatment for Both Countries, (',
                 weeks,')'),
  type = "latex")  %>% 
...
print(table)
cat(table, file = paste0("../../results/04-analysis/",country,"/endline_table_b1_fake_all_", 
                         stage,".tex"))
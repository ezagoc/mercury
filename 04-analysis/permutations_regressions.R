...
# Read df (Aggregated Follower Data Set)
data <- read_parquet(paste0('../../data/03-experiment/',country,
                            '/treatment/followers/01-preprocess/aggregated/stage1.parquet'))
...
# Read the follower assignment permutations:

followers <- read_parquet(paste0("../../data/04-analysis/",country,
                                 "/ties",i,".parquet"), 
                          as_tibble = TRUE)

# Run the first 250 permutations (with the 1000 my computer crashed, so I do it 250 each 4 times)
a <- 250*i + 1
b <- 250*(i+1)
coefs_list <- list()
for (x in a:b) {
  ...
  # Function to pool treatment variables
  ...
  count <- 1
  for (au in aux) {
    # t_strong: treated influecers followed strongly
    # c_t_strong_total: total number of influencers followed strongly, treated and control
    # baseline control added, stratification and block 1 (ads treatment) FEs
    fmla1 <- as.formula(paste0(au, "~ t_strong + t_weak + t_neither + ads_treatment + ",
                               au, "_base  | strat_block1 + c_t_strong_total + c_t_weak_total + c_t_neither_total"))
...
# Export the coefficients to obtain the p-value in Dani's Code
write_xlsx(coefs_perm, paste0("../../data/04-analysis/",country,
                              "/pestimates_joint_endline_filter", i,".xlsx"))
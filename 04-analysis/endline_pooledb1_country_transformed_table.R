...
if (type == 'log_'){
  ...
  dep_var_new <- c("\\shortstack{arcsinh( Verifiable \\\\ RTs )}", 
...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_b1_good_", base,"_", type, ".xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_b1_good_",stage,"", base, type),
  header = FALSE,
  ...
  title = paste0('Panel A, Pooled (Batch 1) Endline Results for the SMI Treatment for Both Countries ', 
                 '(', weeks,' ', base,')'),
  type = "latex")  %>% 
...
print(table)
cat(table, file = paste0("../../results/04-analysis/",country,"/", type,"endline_table_b1_",
                         base, stage,".tex"))

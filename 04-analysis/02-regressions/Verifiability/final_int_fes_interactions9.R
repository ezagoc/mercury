...
file_code <- 'intensive_interactions8'
...
for (stage in list_stages){
  ...
  df <- get_analysis_ver_final_winsor(stage = stage, batches = 'b1b2',
                                      initial_path = '../../../../') |>
    filter(n_posts_base>0) |> filter(total_influencers<9)
...
for (type in list_types){
  ...
  writexl::write_xlsx(final, paste0(ini, 'EstimatesFinal/',type,'verifiability_intensive_interactions13.xlsx'))

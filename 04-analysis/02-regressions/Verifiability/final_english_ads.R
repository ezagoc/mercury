...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    count2 <- 1
    for (x in aux) {
      fmla1 <- as.formula(paste0(x, "~ ads_treatment + dummy_eng_prop + ", x, 
                                 "_base  | strat_block1"))
    ...
    df_coef <- tibble(coef = coef, sd = se, var = aux_eng) |> 
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'eng_prop' ~ paste0(addon, 'Proportion of English RTs + Posts'),
                                var == 'n_eng_prop' ~ paste0(addon, 'Proportion of Non-English RTs + Posts')),

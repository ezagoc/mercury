...
reg <- feols(as.formula(paste0('log_verifiability', "~ total_treated + ",
                               'log_verifiability', "_base + ", int_fes, ' + ', int_cols, 
                               "| total_influencers")), 
             data = df)

coefs <- coeftable(reg)

coefs <- cbind('treatment' = rownames(coefs), coefs) |> as_tibble()

coll <- tibble(treatment = reg$collin.var)

coefs <- bind_rows(coefs, coll) |> arrange(treatment)

writexl::write_xlsx(coefs, 'interactions_coefs.xlsx')
# Assuming 'data' is your data frame and 'variable' is the categorical variable


means_f <- df |> select(starts_with('tao_')) |> summarise(across(everything(), ~mean(.x)))
...
for (type in list_types){
  ...
  writexl::write_xlsx(final, paste0(ini, 'EstimatesFinal/',type,'verifiability_intensive_interactions.xlsx'))
  ...
  if (addon == 'log '){
    results_plot <- results_plot + ylim(-13, 5)
  }else{

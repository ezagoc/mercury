...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    coefs_perm <- data.frame(eng, n_eng, eng1, n_eng1, eng2, n_eng2)
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'eng' ~ paste0(addon, 'English RTs + Posts'),
                                var == 'n_eng' ~ paste0(addon, 
                                                          'Non English RTs + Posts'),
                                var == 'eng1' ~ paste0(addon, 'English RTs'),
                                var == 'n_eng1' ~ paste0(addon, 'Non English RTs'),
                                var == 'eng2' ~ paste0(addon, 'English Posts'), 
                                var == 'n_eng2' ~ paste0(addon, 'Non English Posts')), 
           Stage = case_when(stage == 'stage1_2' ~ 'Weeks 1-4',
                             stage == 'stage3_4' ~ 'Weeks 5-8',
                             stage == 'stage5_6' ~ 'Weeks 9-12'))
  
  final$Variable <- factor(final$Variable, levels = c(paste0(addon, 'English RTs + Posts'),
                                                      paste0(addon, 
                                                             'Non English RTs + Posts'), 
                                                      paste0(addon, 'English RTs'), 
                                                      paste0(addon, 'Non English RTs'), 
                                                      paste0(addon, 'English Posts'), 
                                                      paste0(addon, 'Non English Posts')))

...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    df_coef <- tibble(coef = coef, sd = se, var = aux_s_v_tot) |> 
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'pos_v_covid' ~ paste0(addon, 'Positive COVID Posts + Shares'),
                                var == "neutral_v_covid" ~ paste0(addon, 
                                                                  'Neutral COVID Posts + Shares'),
                                var == "neg_v_covid" ~ paste0(addon, 
  ...
  if (addon == 'log '){
    results_plot <- results_plot + ylim(-.006, .006)
  }else{

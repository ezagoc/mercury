...
for (stage in list_stages){
  ...
  df <- get_analysis_sent_final_winsor(stage = stage, batches = 'b1b2',
                                            initial_path = '../../../../')
  ...
  for (type in list_types){
    ...
    df_coef <- tibble(coef = coef, sd = se, var = aux_s_v_tot2) |> 
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == "pos_v_vax" ~ paste0(addon, 'Positive COVID Vaccine Posts + Shares'),
                                var == "neutral_v_vax" ~ paste0(addon, 
                                                                   'Neutral COVID Vaccine Posts + Shares'),
                                var == "neg_v_vax" ~ paste0(addon, 
                                                               'Negative COVID Vaccine Posts + Shares'),
                                var == "n_posts_vax" ~ paste0(addon, 
  ...
  if (addon == 'log '){
    results_plot <- results_plot + ylim(-.0025, .0025)
  }else{

  ...
  for (type in list_types){
    ...
    write_xlsx(
      coefs_perm, paste0("../../../../data/04-analysis/",country,
                         "/",stage,"/original/", data_type, '/', type, li,
                         "next_nbase0.xlsx"))
    ...
    write_xlsx(coefs_fin, paste0("../../../../data/04-analysis/joint/", stage, 
                                 "/permutations/", data_type, '/', type, li,
                                 "next_nbase0.xlsx"))
  }
  print(stage)
}
...
for (li in list_cutoffs){
for (type in list_types){
  file_coefs <- paste0(type, li, file_code)
  ...
  ggsave(results_plot, 
         filename = paste0('../../../../results/01-regression_graphs/',
                           data_type, '/', type, li, file_code, '_RT.pdf'), 
  ...
  ggsave(results_plot, 
         filename = paste0('../../../../results/01-regression_graphs/', 
                           data_type, '/', type, li, file_code, '_post.pdf'), 
         device = cairo_pdf, width = 8.22, height = 6.59, units = 'in')
}
}

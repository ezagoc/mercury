...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == aux_aux[1] ~ paste0(addon, 'Total Likes (Verifiable)'),
                                var == aux_aux[2] ~ paste0(addon, 'Total Likes (True)'),
                                var == aux_aux[3] ~ paste0(addon, 'Total Likes (Fake)'),
                                var == aux_aux[4] ~ paste0(addon, 'Total Likes (English)'),

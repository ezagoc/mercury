...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == aux_aux[1] ~ paste0(addon, 'Total Comments (Verifiable)'),
                                var == aux_aux[2] ~ paste0(addon, 'Total Comments (True)'),
                                var == aux_aux[3] ~ paste0(addon, 'Total Comments (Fake)'),
                                var == aux_aux[4] ~ paste0(addon, 'Total Comments (English)'),

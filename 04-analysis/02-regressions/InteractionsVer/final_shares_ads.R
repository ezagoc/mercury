...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == aux_aux[1] ~ paste0(addon, 'Total Shares (Verifiable)'),
                                ...
                                var == aux_aux[4] ~ paste0(addon, 'Total Shares (English)'),
  ...
  final$Variable <- factor(final$Variable, levels = c(paste0(addon, 'Total Shares (English)'),
                                                      paste0(addon, 
                                                             'Total Shares (Non-Verifiable)'), 
                                                      paste0(addon, 'Total Shares (Verifiable)'), 
                                                      paste0(addon, 'Total Shares (True)'), 
                                                      paste0(addon, 'Total Shares (Fake)')))

...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'ver_rt' ~ paste0(addon, 'Total Reactions (Verifiable)'),
                                var == 'true_rt' ~ paste0(addon, 'Total Reactions (True)'),
                                var == 'fake_rt' ~ paste0(addon, 'Total Reactions (Fake)'),
                                var == 'tot' ~ paste0(addon, 'Total Reactions (English)'),
  ...
  writexl::write_xlsx(final, paste0(ini, 'EstimatesFinal/',type, data_type, 'reac_intensive.xlsx'))

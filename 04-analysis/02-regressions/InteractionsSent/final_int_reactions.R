...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    coefs_perm <- data.frame(ver_rt, true_rt, fake_rt, n_ver_rt)
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'ver_rt' ~ paste0(addon, 'Total Reactions COVID Posts'),
                                var == 'true_rt' ~ paste0(addon, 'Total Reactions Positive COVID Posts'),
                                var == 'fake_rt' ~ paste0(addon, 'Total Reactions Neutral COVID Posts'),
                                var == 'n_ver_rt' ~ paste0(addon, 
                                                           'Total Reactions Negative COVID Posts')),
           Stage = case_when(stage == 'stage1_2' ~ 'Weeks 1-4',
                             stage == 'stage3_4' ~ 'Weeks 5-8',
                             stage == 'stage5_6' ~ 'Weeks 9-12'))
  
  final$Variable <- factor(final$Variable, levels = c(paste0(addon, 'Total Reactions COVID Posts'),
                                                      paste0(addon, 
                                                             'Total Reactions Positive COVID Posts'), 
                                                      paste0(addon, 'Total Reactions Neutral COVID Posts'), 
                                                      paste0(addon, 'Total Reactions Negative COVID Posts')))
  
  writexl::write_xlsx(final, paste0(ini, 'EstimatesFinal/',type, data_type, '_reac_intensive.xlsx'))

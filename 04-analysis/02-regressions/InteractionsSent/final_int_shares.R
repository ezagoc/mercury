...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    aux <- paste0(type, aux_sent_share)
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'ver_rt' ~ paste0(addon, 'Total Shares COVID Posts'),
                                var == 'true_rt' ~ paste0(addon, 'Total Shares Positive COVID Posts'),
                                var == 'fake_rt' ~ paste0(addon, 'Total Shares Neutral COVID Posts'),
                                var == 'n_ver_rt' ~ paste0(addon, 
                                                           'Total Shares Negative COVID Posts')),
           Stage = case_when(stage == 'stage1_2' ~ 'Weeks 1-4',
                             stage == 'stage3_4' ~ 'Weeks 5-8',
                             stage == 'stage5_6' ~ 'Weeks 9-12'))
  
  final$Variable <- factor(final$Variable, levels = c(paste0(addon, 'Total Shares COVID Posts'),
                                                      paste0(addon, 
                                                             'Total Shares Positive COVID Posts'), 
                                                      paste0(addon, 'Total Shares Neutral COVID Posts'), 
                                                      paste0(addon, 'Total Shares Negative COVID Posts')))

...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == aux_aux[1] ~ paste0(addon, 'Total Reactions COVID Posts'),
                                var == aux_aux[2] ~ paste0(addon, 'Total Reactions Positive COVID Posts'),
                                var == aux_aux[3] ~ paste0(addon, 'Total Reactions Neutral COVID Posts'),
                                var == aux_aux[4] ~ paste0(addon, 'Total Reactions Negative COVID Posts')),
  ...
  writexl::write_xlsx(final, paste0(ini, 'EstimatesFinal/',type, data_type, '_reac_ads.xlsx'))

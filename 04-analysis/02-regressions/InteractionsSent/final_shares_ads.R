...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == aux_aux[1] ~ paste0(addon, 'Total Shares COVID Posts'),
                                var == aux_aux[2] ~ paste0(addon, 'Total Shares Positive COVID Posts'),
                                var == aux_aux[3] ~ paste0(addon, 'Total Shares Neutral COVID Posts'),
                                var == aux_aux[4] ~ paste0(addon, 'Total Shares Negative COVID Posts')),

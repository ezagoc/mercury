...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == "pos_b_vax" ~ paste0(addon, 'Positive COVID Vaccine Posts + Shares'),
                                var == "neutral_b_vax" ~ paste0(addon, 
                                                           'Neutral COVID Vaccine Posts + Shares'),
                                var == "neg_b_vax" ~ paste0(addon, 
  ...
  if (addon == 'log '){
    results_plot <- results_plot + ylim(-.002, .002)
  }else{

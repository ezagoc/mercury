...
file_code <- 'ext_nbase0_posts_rts_ads'
...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    df_coef <- tibble(coef = coef, sd = se, var = aux_urls) |> 
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'total_urls' ~ paste0(addon, 'Total URLs'),
                                var == 'total_info' ~ paste0(addon, 'Total Info.'),
                                var == 'total_news' ~ paste0(addon, 'Total News'),
                                var == 'fact_check' ~ paste0(addon, 'Fact-Checks'),
                                var == 'rel_news' ~ paste0(addon, 'Reliable'),
                                var == 'non_rel_news' ~ paste0(addon, 'Non-Reliable'),
                                var == 'other' ~ paste0(addon, 'Other')),

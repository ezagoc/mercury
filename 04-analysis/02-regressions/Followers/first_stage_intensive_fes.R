...
country <- 'joint'
data_type <- 'Followers'
file_code <- 'first_stage_intensive_fes'
...
mmm <- 'dsmi significant at the 10% sig, ads, dads, smi not significant'
...
table <- stargazer(
  ...
  add.lines = list(c("Baseline control", rep("No", 4)), 
                   c("Block1 FEs", rep("No", 4)), 
                   c("Total Influencer FEs", rep("No", 4)),
                   c("Pure control mean", round(means, 4))
  ),
  title = 'First Stage, Intensive Margin Analysis (NO FEs)',
  type = "latex")
...
print(table)
cat(table, file = paste0("../../../../results/01-regression_graphs/",data_type,"/intensive_first_stage_fe.tex"))
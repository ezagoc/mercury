...
for (stage in list_stages){
  ...
  for (type in list_types){
    ...
    df_coef <- tibble(coef = coef, sd = se, var = aux_rt) |> 
...
for (type in list_types){
  ...
  final <- final |> 
    mutate(Variable = case_when(var == 'interaction_smi' ~ paste0(addon, "Number of SMI's Tweets Shared"),
                                var == 'interaction_ac' ~ paste0(addon, 

...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_b1_b2_p_all.xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_b1_b2_p_fake_all",stage,"", country),
  header = FALSE,
  ...
  title = paste0('Panel C, Pooled (Batch 1, 2 and Pilot) Endline Results for the SMI Treatment in ', 
                 country,
                 ' , (', weeks,')'),
  type = "latex")  %>% 
...
print(table)
cat(table, file = paste0("../../results/04-analysis/",stage,"/endline_table_b1_b2_p_fake__all_", 
                         country,".tex"))
...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_sentiments_b1b2p.xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_sent_b1b2p_",stage, country),
  header = FALSE,
  ...
  title = paste0('Panel A, Pooled Sentiment Analysis Results (Pilot, Batch 1 and 2) for the SMI Treatment in Both Countries, (',
                 weeks,')'),
  type = "latex")  %>% 
...
cat(table, file = paste0("../../results/04-analysis/", country,
                         "/endline_table_sent_b1b2p_", stage,".tex"))

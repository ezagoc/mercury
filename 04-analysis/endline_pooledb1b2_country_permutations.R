...
bots_ke1 <- read_parquet(paste0('../../data/04-analysis/KE/bots_batch1.parquet')) |>
  rename(follower_id = author_id)

bots_ke2 <- read_parquet(paste0('../../data/04-analysis/KE/bots_batch2.parquet')) |>
  rename(follower_id = author_id)

bots <- rbind(bots_ke1, bots_ke2)

bots <- bots[!duplicated(bots$follower_id), ]

baseline_ke <- read_parquet('../../data/04-analysis/KE/baseline_months.parquet')

df_ke <- df_ke |> left_join(fake_ke, by = c('follower_id', 'batch_id')) |> 
  left_join(bots_ke) |> left_join(baseline_ke) |> filter(is.na(dummy_1) == T) |>
  mutate(pais = 'KE')
...
for (m in 1:1000){
  ...
  write_xlsx(coefs_perm, paste0("../../data/04-analysis/",country, "/",
                                stage, "/pestimates_ver/country_b1b2_", 
                                a,".xlsx"))

...
data <- data |> mutate(strat_block1 = paste0(strat_block1, pais, batch_id))
...
table <- stargazer(
  ...
  label = paste0("tab:pooled_ads_b1_b2_p_fake_",stage,"", country),
  header = FALSE,
  ...
  add.lines = list(c('Baseline Filtered', rep(c('No', 'Yes'), 8)),
                   c("Strat. Block1 FE", rep("Yes", 16)),
                   c("Baseline control", rep("Yes", 16)),
                   c("Outcome mean", means),
                   c("Outcome std. dev.", sds),
                   c("Outcome range", range)),
  title = paste0('Panel B, Pooled (Batch 1, 2 and Pilot) Endline Results for the Ads Treatment for Both Countries ', 
                 '(', weeks,')'),
  type = "latex")

note.latex <- paste0("\\multicolumn{17}{l} {\\parbox[t]{26cm}{ \\textit{Notes:}
...
print(table)
cat(table, file = paste0("../../results/04-analysis/", country, "/endline_ads_b1_b2_p_fake_", 
                         stage,".tex"))
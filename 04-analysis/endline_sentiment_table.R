...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_sentiment.xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_sent_",stage,"", country),
  header = FALSE,
  ...
  title = paste0('Panel A, Sentiment Analysis Results for the SMI Treatment in ',
                 country, ' , (', weeks,')'),
  type = "latex")  %>% 
...
print(table)
cat(table, file = paste0("../../results/04-analysis/endline_table_sent_", stage,"_", 
                         country,".tex"))
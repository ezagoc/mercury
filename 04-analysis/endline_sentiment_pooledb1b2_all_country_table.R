...
pvals = read_excel(paste0("../../data/04-analysis/", country, 
                          "/",stage,"/agg_pvalues_sentiment_b1b2_all.xlsx"))
...
table <- stargazer(
  ...
  label = paste0("tab:endline_table_sent_b1b2_all_",stage),
  header = FALSE,
  ...
  title = paste0('Panel C, Pooled Sentiment Analysis Results (Batch 1 and 2) for the SMI Treatment in  Both Countries', 
                 '(',weeks,')'),
  type = "latex") %>% 
...
cat(table, file = paste0("../../results/04-analysis/", country,
                         "/endline_table_sent_b1b2_all_", stage,".tex"))
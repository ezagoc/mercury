...
path_out <- paste0('../../data/03-experiment/',country,
                   '/treatment/followers/00-raw/tweets_batch2/may/')
path_no_tweets <- paste0('../../data/03-experiment/',country,
                         '/treatment/followers/00-raw/tweets_batch2/no_tweets_may/')
...
ids <- setdiff(ids, c('1387345676032528386', '1387966932310269954', '1490939801817817094', '1520092735113334785', '1529997307462180864', '1533395818232877057', '1546020385145094144', '1601404143217922048', '1644806389', '1680219109', '2720801911', '721412732067647492', '80292927', '807811500408770560', '1125942599716569089'))
ids <- sort(ids)
...
get_time_export <- function(id_user){
  ...
  if (length(df_prueba)==0){
    ...
    df_prueba <- df_prueba |> unnest(public_metrics) |> 
      select(author_id, id, conversation_id, created_at, text,
...
ids[2:length(ids)] |> map(~ get_time_export(.x))
